...
int Socket::Write( const char* text ) { //escribir en la cola de solicitudes
	int resp;
	int len = strlen(text);
 ...
 */
int Socket::Listen( int queue ) { //BIEN
	int resp = listen(idSocket,queue);
	if(-1 == resp){
...
int Socket::Bind( int puerto ){
	...
	int resp = bind(idSocket, (struct sockaddr *)& address, sizeof(address)); //asocia el socket con el puerto
	if(-1 == resp){
...
Socket * Socket::Accept(){
	...
	int idSocket2 = accept(idSocket, (struct sockaddr *)&address,(socklen_t*) &addrlen);
	if(-1 == idSocket2){
		perror("ERROR AL ACEPTAR");
	}
	Socket * s2 = new Socket();
	s2->SetIDSocket(idSocket2);
    return s2;
...
int Socket::Shutdown( int mode ) {
	int resp = shutdown(idSocket, mode);
	if(-1 == resp){
